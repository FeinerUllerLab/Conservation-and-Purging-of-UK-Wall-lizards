...
mkdir -p /home/feiner/Projects/UKwallies/Results_RoH_Het_W-EUR/500kb
cd /home/feiner/Projects/UKwallies/Results_RoH_Het_W-EUR/500kb
...
awk -v minsize="$RoH_size_num" '$6 > minsize' roh.pseudo.qual > roh.pseudo.qual.${RoH_size_nam}
...
while read -r sample; do
    ...
    sum=$(awk -v s="$sample" '$2 == s {total += $6} END {print total+0}' roh.pseudo.qual.${RoH_size_nam})

    # Count number of ROH segments for this sample
    count=$(awk -v s="$sample" '$2 == s {n++} END {print n+0}' roh.pseudo.qual.${RoH_size_nam})
